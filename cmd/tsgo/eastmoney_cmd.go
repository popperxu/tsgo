...
// NewEastmoneyAPICmd returns the root command
func NewEastmoneyAPICmd() *cobra.Command {
	...
	rootCmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			//}
			screen := TerminalStocks.NewScreen(TerminalStocks.API_VENDOR_EASTMONEY)
			defer screen.Close()

			profile := TerminalStocks.NewProfile(TerminalStocks.API_VENDOR_EASTMONEY)
